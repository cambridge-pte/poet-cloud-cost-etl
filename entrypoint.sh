...
echo "${SYNC_SCHEDULE:-0 2 * * *} root cd /app && /usr/local/bin/python src/main.py sync --months ${SYNC_MONTHS:-1} >> /var/log/etl/sync.log 2>&1" > /etc/cron.d/etl-sync
...
if [ "${RUN_ON_START:-true}" = "true" ]; then
    ...
    python src/main.py sync --months ${SYNC_MONTHS:-1}
fi
